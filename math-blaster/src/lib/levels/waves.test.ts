...
describe('buildFormation', () => {
  ...
  it('keeps every slot inside the reachable lanes, for every shape', () => {
    ...
    for (const shape of shapes) {
      for (let waveIndex = 0; waveIndex < 12; waveIndex++) {
        for (const slot of buildFormation(spec({ shape, archetypes: Array(5).fill('drifter') }), waveIndex)) {
  ...
  describe('column', () => {
    ...
    it('moves its lane between waves so a looping plan does not drill one spot', () => {
      const lanes = new Set([0, 1, 2, 3].map((i) => buildFormation(spec({ shape: 'column' }), i)[0].xPct));
  ...
  describe('pincer', () => {
    it('splits to the edges and leaves the middle open', () => {
      const slots = buildFormation(spec({ shape: 'pincer', archetypes: ['drifter', 'drifter'] }), 0);
  ...
  describe('scatter', () => {
    it('varies both lane and depth across slots', () => {
      const slots = buildFormation(spec({ shape: 'scatter', archetypes: Array(4).fill('drifter') }), 2);
