...
export function spriteSize(key: SpriteKey, scale: number = SPRITE_META[key].scale): { width: number; height: number } {
  const meta = SPRITE_META[key];
 ...
 */
function drawSilhouette(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void {
  ctx.save();
