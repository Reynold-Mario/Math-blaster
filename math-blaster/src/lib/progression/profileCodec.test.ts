...
describe('records take the high-water mark', () => {
  it('keeps the furthest wave either side reached', () => {
    const merged = mergeProfiles(profile({ highestWaveReached: 31 }), profile({ highestWaveReached: 12 }), 'b-is-newer');
...
describe('preferences take the newer write', () => {
  it('follows the hint for the selected grade', () => {
    ...
    expect(mergeProfiles(profile({ selectedGrade: '3' }), profile({ selectedGrade: 'K' }), 'b-is-newer').selectedGrade).toBe('K');
    expect(mergeProfiles(profile({ selectedGrade: '3' }), profile({ selectedGrade: 'K' }), 'a-is-newer').selectedGrade).toBe('3');
  });
});
...
describe('merging with nothing', () => {
  ...
  it('is idempotent against itself', () => {
    const real = profile({ currency: 20, earnedTotal: 20, skillProgress: { bounty: 1 }, highestWaveReached: 9 });
...
describe('an existing player, through the real store', () => {
  ...
  function storeWith(raw: string) {
    ...
    return {
      map,
      handle: createLocalStorageStore({ storage, keyFor: () => PROFILE_STORAGE_KEY }).open(profileCodec),
    };
  }
