<script lang="ts">
	...
	const currentTheme = $derived(
		(isBossPhase && runtime.bossRules?.theme
			? runtime.bossRules.theme
			: backdropForWave(runtime.waveNumber)) as Backdrop
	);
	...
	const waveProgressPct = $derived(
		runtime.waveSize > 0 ? (waveCleared / runtime.waveSize) * 100 : 0
	);
	...
	const surviveSeconds = $derived(
		isBossPhase ? Math.max(0, Math.ceil(runtime.boss!.surviveRemainingMs / 1000)) : 0
	);
	...
	function purchaseSkill(node: SkillNode<BaseSkillEffect>) {
		const result = purchaseNextInstallment(
			node,
...
<div class="game-root">
	...
	{:else}
			...
			<div class="hud-mid">
				...
				{#if isBossPhase}
					<div class="bar boss">
						<div
							class="fill"
							style="width:{(runtime.boss!.surviveRemainingMs / runtime.boss!.surviveTotalMs) *
								100}%"
						></div>
					</div>
					...
					<div class="boss-phase">
						{runtime.boss!.inFinale ? 'FINAL ATTACK' : bossPhaseName || runtime.boss!.name}
					</div>
				{:else}
		...
		>
			<GameCanvas {runtime} theme={currentTheme} {reducedMotion} />
			...
			<div class="controls">
				<div class="move-controls">
					<button
						...
						onpointerleave={() => input.releaseMove('left')}>◀</button
					>
					<button
						class="move-btn"
						onpointerdown={() => input.pressMove('right')}
						onpointerup={() => input.releaseMove('right')}
						onpointerleave={() => input.releaseMove('right')}>▶</button
					>
				</div>
				<div class="keypad">
