...
describe('tallying a run', () => {
	it('counts every answer as an attempt and only exact/equivalent as correct', () => {
		...
		expect(live).toEqual([
			{ topicId: 'g1-add-sub-10', standardCode: undefined, attempts: 6, correct: 2 }
		]);
	});
...
describe('run boundaries', () => {
	it('hands the tally over on game-over and starts clean', () => {
		...
		expect(finished).toEqual([
			[{ topicId: 'a', standardCode: undefined, attempts: 1, correct: 1 }]
		]);
		...
		expect(finished[1]).toEqual([
			{ topicId: 'b', standardCode: undefined, attempts: 1, correct: 0 }
		]);
		rec.dispose();
	});
