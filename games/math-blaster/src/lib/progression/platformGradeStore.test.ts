...
describe('createPlatformGradeStore', () => {
  it('is a transparent pass-through with no platform configured', () => {
    ...
    const handle = createPlatformGradeStore({ inner: inner.store, identity: null, mapGrade: (g) => g }).open(codec);
  ...
  it('never writes through the cache', async () => {
    const inner = fakeInner();
    createPlatformGradeStore({ inner: inner.store, identity: identified('3'), mapGrade: (g) => g }).open(codec);
    await settled();
  ...
  it('says nothing at all when the grade is absent or unusable', async () => {
    for (const [identity, mapGrade] of [
      ...
      const inner = fakeInner();
      const handle = createPlatformGradeStore({ inner: inner.store, identity, mapGrade }).open(codec);
      const seen: State[] = [];
      handle.onRemote((s) => void seen.push(s));
      await settled();
