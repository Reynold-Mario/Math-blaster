...
function parseHousehold(body: unknown): HouseholdLearner[] | null {
	...
	for (const row of rows) {
		...
		parsed.push({
			id: entry.id,
			grade:
				typeof entry.grade === 'string' && PLATFORM_GRADES.includes(entry.grade)
					? entry.grade
					: null,
...
export function createVtIdentity(options: VtIdentityOptions): LearnerIdentitySource {
	...
	async function load(): Promise<IdentityResult> {
		...
		try {
			...
			if (!session.ok)
				return anonymous(
					session.status === 401 || session.status === 403 ? 'unauthenticated' : 'unavailable'
				);
			if (!isSignedIn(session.body)) return anonymous('unauthenticated');
			if (!household.ok)
				return anonymous(
					household.status === 401 || household.status === 403 ? 'unauthenticated' : 'unavailable'
				);
