...
function fakeFetch(replies: Record<string, Reply>) {
	...
	const impl = (async (input: RequestInfo | URL) => {
		...
		return {
			...
			headers: {
				get: (k: string) =>
					k.toLowerCase() === 'content-type' ? (reply.contentType ?? null) : null
			},
...
function resolveWith(replies: Record<string, Reply>, search = ''): Promise<IdentityResult> {
	return createVtIdentity({
		base: '/learner',
		fetchImpl: fakeFetch(replies).impl,
		search
	}).resolve();
}
...
describe('createVtIdentity', () => {
	...
	it('reports unauthenticated for a rejected read and for a signed-out session', async () => {
		await expect(
			resolveWith({ [SESSION_URL]: { status: 401 }, [LEARNERS_URL]: { status: 401 } })
		).resolves.toEqual({
			outcome: 'anonymous',
		...
		await expect(
			resolveWith({
				[SESSION_URL]: json({ session: null, user: null }),
				[LEARNERS_URL]: household()
			})
	...
	it('reports no-learners for an empty household', async () => {
		await expect(
			resolveWith({ [SESSION_URL]: SIGNED_IN, [LEARNERS_URL]: household() })
		).resolves.toEqual({
			outcome: 'anonymous',
			reason: 'no-learners'
		});
