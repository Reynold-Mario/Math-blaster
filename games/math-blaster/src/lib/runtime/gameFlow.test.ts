...
function startAtArchetype(archetype: EnemyArchetypeId): {
	state: RuntimeState;
...
function spawnUntil(state: RuntimeState, profile: PlayerProfile, archetype: string): EnemyInstance {
	tickUntil(
		state,
		profile,
		() => state.enemies.some((e) => e.archetype === archetype),
		`a ${archetype} to spawn`
	);
...
describe('grade scoping', () => {
	...
	function problemsAtGrade(grade: GradeLevel, waves: number[]): ProblemDefinition[] {
		...
		for (const wave of waves) {
			...
			tickUntil(
				state,
				profile,
				() => state.enemies.length > 0 || state.boss !== null,
				`wave ${wave}`
			);
	...
	it('does reach multiplication for a Grade 3 run', () => {
		...
		const operators = new Set(
			problemsAtGrade('3', [1, 2, 3, 4, 6, 7, 8, 9, 11, 12]).map((p) => operatorOf(p))
		);
...
describe('wave spawning', () => {
	...
	it('spawns exactly the archetypes the wave ladder authors', () => {
		const { state, profile } = startAtWave(1);
		tickUntil(state, profile, () => state.enemies.length > 0, 'the first wave');
		expect(state.enemies.map((e) => e.archetype).sort()).toEqual(
			[...waveSpecFor(1).archetypes].sort()
		);
	});
	...
	function masterBossAt(waveNumber: number) {
		...
		for (let i = 0; i < fight.boss.comboRequired; i++) {
			...
			shoot(
				fight.state,
				fight.profile,
				fight.state.boss.xPct,
				correctAnswer(fight.state.boss.problem)
			);
		}
		const defeated = eventsOfType('boss-defeated');
		expect(defeated).toHaveLength(1);
	...
	describe('shields and weak points', () => {
		...
		it('raises its shield when a shielded phase runs out of open time', () => {
			...
			tickUntil(
				state,
				profile,
				() => state.boss !== null && !state.boss.vulnerable,
				'the shield to go up'
			);
