...
function submitAnswer(
	...
	else if (target.kind === 'boss' || target.kind === 'boss-weak-point')
		problem = state.boss!.problem;
	if (!problem) return;
...
export function sweep(runsPerPlayer: number, options: SimOptions = {}): Summary[] {
	return SIM_PLAYERS.map((player) => {
		...
		for (let i = 0; i < runsPerPlayer; i++)
			runs.push(simulateRun(player, 1000 + i * 7919, options));
