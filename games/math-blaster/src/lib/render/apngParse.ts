...
export function parseApng(bytes: Uint8Array): ParsedApng {
  ...
  const playCount = actl ? new DataView(actl.data.buffer, actl.data.byteOffset, actl.data.byteLength).getUint32(4) : 1;
  ...
  interface Pending {
    ...
    delayMs: number; disposeOp: number; blendOp: number;
    parts: Uint8Array[];
  }
  ...
  const frames = pendings
    ...
    .map((p) => ({
      ...
      delayMs: p.delayMs, disposeOp: p.disposeOp, blendOp: p.blendOp,
    }));
...
function rebuild(format: Uint8Array, w: number, h: number, carried: Uint8Array[], parts: Uint8Array[]): Uint8Array<ArrayBuffer> {
  return concat([
