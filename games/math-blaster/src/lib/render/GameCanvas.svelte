<script lang="ts">
	...
	function handleGameEvent(event: GameEvent) {
		switch (event.type) {
			...
			case 'wave-announced':
				pushBanner(
					event.isBoss ? `WAVE ${event.waveNumber} - BOSS` : `WAVE ${event.waveNumber}`,
					event.isBoss ? COLOR_MISS : COLOR_INFO
				);
	...
	function drawOneShots(ctx: CanvasRenderingContext2D, nowMs: number) {
		...
		for (const fx of oneShots) {
			...
			drawSprite(
				ctx,
				fx.sprite,
				px(fx.xPct, LOGICAL_W),
				px(yPct, LOGICAL_H) - size.height / 2,
				fx.scale,
				{
					centerX: true,
					frame,
